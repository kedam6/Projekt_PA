...
unsigned int* SdlUtils::SdlInit(int height, int width, const char* title, int fps)
{
	pixelbuffer = new unsigned int[height*width];

	SDL_EnableScreenSaver();
	
	window = SDL_CreateWindow(title, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, height, width, SDL_WINDOW_SHOWN);
	renderer = SDL_CreateRenderer(window, -1, SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC);
	texture = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_ABGR8888, SDL_TEXTUREACCESS_STATIC, height, width);
...
int SdlUtils::SdlLimitFps(int * limiter, int fps)
{
	if (*limiter == 0)
	{
